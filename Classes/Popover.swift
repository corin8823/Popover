...
open class Popover: UIView {
    ...
    open func show(_ contentView: UIView, fromView: UIView, inView: UIView) {
        ...
        let absoluteFrame = fromView.convert(fromView.bounds, to: fromView.window)
        
        if self.popoverType == .down && absoluteFrame.bottom + contentView.height > UIScreen.main.bounds.height {
            self.popoverType = .up
        }
        
        if self.popoverType == .up && absoluteFrame.top - contentView.height < 0 {
